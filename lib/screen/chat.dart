      ...
      child: Row(
        children: <Widget>[
          ...
          IconButton(
            ...
            color: Theme.of(context).primaryColor,
            onPressed: () {
              myInputController.text.isNotEmpty
                  ? publishMyMessage()
                  : ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Please write a message to send")));;
            },
          ),
        ],
      ),
          ...
          actions: <Widget>[
            IconButton(
              ...
              onPressed: () {
                Navigator.push(
                  context,
                  PageRouteBuilder(pageBuilder: (_, __, ___) => const ChatScreenReceiver(), transitionDuration: const Duration(seconds: 0))
                );
              },
            ),
          ],
